...
// Camera stuff
function attachCamera(element, pose, renderer) {
...
function start_here() {
    ...
    .then(function(raw_data) {
        ...
        var a = 0;
        //var eye = vec3.add(vec3.create(), center, vec3.multiply(vec3.create(), tsdf.size, vec3.fromValues(0.5 * Math.sin(a), 0.5*-Math.cos(a), 0.0)));
        var eye = vec3.fromValues(0.5, 0, 0.5);

        var up = vec3.fromValues(0, 0, 1);
        var pose = mat4.lookAt(mat4.create(), eye, center, up);
        // Overwrite translation here
        pose[12] = eye[0];
        pose[13] = eye[1];
        pose[14] = eye[2];
...
function render_depth(tsdf, pose, K, depth, width, height)
{
    ...
    console.log("correct tra", vec3.fromValues(0.5, -0.1, 0.5));
    console.log("correct rot", mat3.fromValues(1, 0, 0, 0, 0, -1, 0, 1, 0))

    console.log(pose_translation);
...
function render(tsdf, pose) {
    ...
    for (var i = 0; i < img.width * img.height * 4; i += 4) {
        ...
        c++;
        img.data[i + 0] = 0;
        img.data[i + 1] = (v-0.3) * 1200;
        img.data[i + 2] = (v-0.3) * 1200;
        img.data[i + 3] = 0xff;
    }
